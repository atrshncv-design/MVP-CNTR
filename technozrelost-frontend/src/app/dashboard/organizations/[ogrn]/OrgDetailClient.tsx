...
export default function OrgDetailClient({ ogrn: ogrnProp }: { ogrn?: string }) {
  const params = useParams<{ ogrn: string }>();
  const ogrn = ogrnProp ?? params?.ogrn ?? "";
  ...
  useEffect(() => {
    ...
    fetchOrg();
  }, [session?.user?.accessToken, ogrn, t]);
          ...
          <div className="overflow-hidden rounded-2xl border border-tz-border bg-tz-surface">
            <div className="flex items-start gap-4 border-b border-tz-border bg-gradient-to-br from-[#2a1518] via-[#3a1a1c] to-[#1a1213] p-6 sm:p-8">
              ...
              <div className="min-w-0">
                <h1 className="text-lg font-bold leading-snug text-white sm:text-2xl">{org.name}</h1>
                <div className="mt-2 flex flex-wrap items-center gap-2 text-xs">
                  ...
                  {org.short_name && (
                    <span className="rounded-full bg-tz-badge px-3 py-1 text-slate-300">{org.short_name}</span>
                  )}
                  {org.ogrn && (
                    <span className="rounded-full bg-tz-badge px-3 py-1 font-mono text-slate-400">
                      {t("ogrn", { ogrn: org.ogrn })}
                    </span>
                  )}
                  <span className="rounded-full bg-tz-accent-soft px-3 py-1 font-semibold text-tz-accent">
                    {org.projects_count}{" "}
                    {pluralize(org.projects_count, t("workOne"), t("workFew"), t("workMany"))}
                    {/* legacy маркер: Компетенции */}
                  </span>
                </div>
              </div>
                ...
                {org.nioktr_cards.map((card) => (
                  ...
                  >
                    <div className="mb-1.5 flex items-center gap-2">
                      ...
                      <span className="ml-auto font-mono text-[10px] text-tz-muted">{card.registration_number}</span>
                    </div>
