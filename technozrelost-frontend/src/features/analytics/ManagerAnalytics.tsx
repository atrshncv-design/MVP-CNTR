...
export function ManagerAnalytics() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(e instanceof Error && e.message ? e.message : t("mgrLoadFailed"));
    } finally {
      setLoading(false);
    }
  }, [token, t]);
  ...
  const statCards = [
    { label: t("mgrTotalMine"), value: funnel.total },
    { label: t("mgrActive"), value: funnel.active },
    { label: t("mgrDrafts"), value: funnel.draft + funnel.auto_confirmed },
  ];
  ...
  return (
    <section data-testid="manager-analytics" className="space-y-6">
      ...
      <Funnel funnel={funnel} title={t("mgrFunnelTitle")} testId="manager-funnel" />
      ...
      <div className="tz-card p-5" data-testid="manager-queue">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-semibold text-tz-fg">{t("mgrQueueTitle")}</h3>
          <span className="tz-badge tz-badge-neutral">{projects.length}</span>
        </div>
        <p className="mt-1 text-xs text-tz-muted">{t("mgrQueueHint", { mark: funnelOk ? "✓" : "✗" })}</p>

        <div className="mt-4 space-y-2">
          {paginated.slice.length === 0 ? (
            <p className="py-6 text-center text-sm text-tz-muted">{t("mgrQueueEmpty")}</p>
          ) : (
        ...
        {paginated.hasMore ? (
          ...
          <p className="mt-4 text-center font-mono text-[11px] text-tz-muted">{t("mgrShownAll")}</p>
        )}
      </div>
