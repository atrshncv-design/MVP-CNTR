...
/**
 * AI-функции в UX/UI — включены (возвращены 2026-09-21 по решению владельца;
 * скрывались флагами в серверной версии).
 * `false` временно прячет пункт «AI-ассистент», страницу /dashboard/ai-assistant
 * (заглушка) и AiDocConsultant в карточке проекта. Код чата, консультанта
 * и словари не удаляются.
 */
export const AI_UI_ENABLED = true;

/**
 * Подбор партнёра — открыт (возвращён 2026-09-21; открывался таском 04,
 * R04/G36: топ-5 через POST /match). `false` временно прячет пункт меню
 * и экран (заглушка). API POST /match и `features/matching/*` не тронуты.
 */
