...
export default function StageProgressPanel({ projectId, currentLevel, status }: { projectId: number; currentLevel: number; status: string }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(errorText(data, t('stageLoadReqError', { status: res.status })));
      const list = data as Requirement[];
      setRequirements(list);
      setSelectedId((current) => current ?? list.find((item) => !item.uploaded)?.id ?? list[0]?.id ?? null);
    } catch (e) { setError(e instanceof Error ? e.message : t('stageLoadReqFailed')); }
    finally { setLoading(false); }
  }, [projectId, status, token, t]);
  ...
  const upload = async (file: File) => {
    if (!token || !selectedId) { setError(t('stageSelectReq')); return; }
    ...
    try {
      ...
      const data = await res.json().catch(() => null);
      if (!res.ok) throw new Error(errorText(data, t('stageUploadDocError', { status: res.status })));
      await load();
      if (data?.request_id) {
    ...
    } catch (e) { setError(e instanceof Error ? e.message : t('stageUploadDocFailed')); }
  ...
  const evaluate = async () => {
    ...
    try {
      ...
      const data = await res.json().catch(() => null);
      if (!res.ok) throw new Error(errorText(data, t('stageEvaluateError', { status: res.status })));
      setEvaluation(data as Evaluation);
    } catch (e) { setError(e instanceof Error ? e.message : t('stageEvaluateFailed')); }
  ...
  return (
    <section className="tz-card p-6" data-od-id="stage-progress">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div><p className="tz-eyebrow">{t('stageEyebrow')}</p><h2 className="tz-card-title mt-1">{t('stageTitle', { from: currentLevel, to: currentLevel + 1 })}</h2><p className="mt-1 text-sm text-tz-muted">{t('stageDesc')}</p>{requirements[0]?.template_version && <p className="mt-1 font-mono text-xs text-tz-secondary">{t('stageTemplateRef', { version: requirements[0].template_version })}</p>}</div>
        <button className="tz-btn tz-btn-secondary" onClick={() => void load()} disabled={loading}><RefreshCw size={15} /> {t('filesRefresh')}</button>
      </div>
      {loading ? <div className="mt-5 h-20 animate-pulse rounded-xl bg-tz-soft" /> : error ? <div className="mt-5 rounded-xl bg-tz-danger-soft p-4 text-sm text-tz-danger">{error}</div> : <>
        ...
        <p className="mt-2 text-xs text-tz-muted">{t('filesHint')}</p>
        <div className="mt-3 flex flex-wrap gap-3"><button className="tz-btn tz-btn-primary" onClick={() => fileRef.current?.click()} disabled={busy || requirements.every((r) => r.uploaded)}>{busy ? <Loader2 className="animate-spin" size={15} /> : <FileUp size={15} />} {t('filesUpload')}</button><button className="tz-btn tz-btn-secondary" onClick={() => void evaluate()} disabled={busy}>{t('stageReevaluate')}</button></div>
        {evaluation && <div className={`mt-4 rounded-xl p-4 ${evaluation.success ? 'bg-tz-success-soft text-tz-success' : 'bg-tz-danger-soft text-tz-danger'}`}><div className="flex items-center gap-2 font-semibold">{evaluation.success ? <CheckCircle2 size={18} /> : <XCircle size={18} />} {evaluation.success ? t('stageEvalPass') : t('stageEvalFail')}</div><p className="mt-1 text-sm">{evaluation.summary || t('stageEvalSaved')}</p>{evaluation.missing.length > 0 && <ul className="mt-2 list-disc pl-5 text-sm">{evaluation.missing.map((item) => <li key={item}>{item}</li>)}</ul>}{evaluation.success && <p className="mt-2 text-sm font-medium">{t('stageEvalSent')}</p>}</div>}
      </>}
    </section>
  );
}
