...
- T07: локальная FastAPI policy скрывает `/docs`, `/redoc`, `/openapi.json` только при `APP_ENV=production`; dev/test сохранили defaults. Focused tests, Ruff, Python 3.11 mypy, два ревью и полный backend suite (725 passed, 1 warning, 485.84 сек) PASS. Deployed docs остаются UNKNOWN, production не менялся.
- T08: malformed HTTP 200 от LLM валидируется отдельно, сохраняет `None` fallback и увеличивает `malformed_total`; HTTP error/timeout остаются своими категориями. Focused 30, AI/chat suite 93 и полный backend suite 744 passed; Ruff, mypy и два ревью PASS. Live provider evaluation ждёт новый ключ владельца; production не менялся.
