...
- ⚠️ **Дефект (передаётся разработчику)**: верифицирующий документ не попадает в `documents` карточки проекта — владелец/участники его не видят (виден только менеджеру внутри заявки). Нужен GET verification-docs или включение в documents карточки
- ⚠️ Ограничение QA: визуальный браузерный прогон не выполнен (browser daemon не стартовал) — QA проведён на API-уровне теми же контрактами, что дёргает фронтенд
