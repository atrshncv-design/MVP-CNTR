...
- Проектирование идёт **вертикально сверху вниз**: бизнес-контуры → системная
  архитектура → доменная модель и БД → API-контракты → ключевые сквозные
  сценарии → интерфейсы → частные функции. Нельзя останавливать архитектурную
  работу ради преждевременной детализации одного экрана или формы.
- Решения уровня конкретных вопросов, локального черновика, видимости отдельных
  AI-полей и micro-UX записываются в decision backlog и принимаются только при
  реализации соответствующего вертикального среза.
...
- **Будущая автоматизация допуска:** решение может быть делегировано LLM только
  ...
  ручного пересмотра.
- **Форма умной заявки в MVP:** фиксированный пошаговый опросник. AI не меняет
  вопросы во время заполнения, а выполняет пост-анализ полного набора ответов:
  классификацию, поиск противоречий, оценку полноты и рекомендации менеджеру.
  Адаптивные AI-вопросы рассматриваются после обратной связи на MVP.
- **Уточнения заявки:** архитектура поддерживает повторные циклы возврата,
  комментарии AI/менеджера, версии ответов и неизменяемую историю. Конкретные
  правила возврата уточняются при проектировании workflow.
- **Показатели карточки проекта:** текущий набор считается предварительным.
  Профильный отдел передаёт окончательные критерии; платформа хранит их как
  версионированную конфигурацию, не зашивая методологические пороги в UI.
- **Воронка:** разделение фиксированного опросника по намерениям потенциально
  применимо как один из первых этапов воронки, но не считается утверждённой
  детализацией формы до реализации intake-контура.
- **Отложенный decision backlog:** заполнение без регистрации/локальный черновик
  и точная видимость AI-оценок заявителю/менеджеру обсуждаются при реализации
  intake и AI-review, а не на архитектурной фазе.
