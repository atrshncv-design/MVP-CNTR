...
## Гигиена контекста и архив документации — 16.09.2026

- ✅ Завершённые прогоны Autopilot до 08.09 перенесены в `.autopilot/archive/runs/`; текущий прогон 15.09 оставлен на верхнем уровне.
- ✅ Исторические интервью, дорожная карта, remediation-планы и старый трекер НИОКТР перенесены в `docs/archive/` без удаления.
- ✅ Архивные зоны исключены из обычного поиска агентов через `.ignore`; прямой доступ по пути и история Git сохранены.
- ✅ Автоматически загружаемый `AGENTS.md` сокращён; подробный справочник вынесен в `docs/agents/project-reference.md`.
