...
## Спека и таски исправлений по аудиту — 08.09.2026

- ✅ Прогон `2026-09-08-audit-remediation` сдан: спека покрывает все подтверждённые находки аудита `aeecb2d`, нарезано 16 тасков в 4 волны (ярус T3), гейты G2/G3/слепая приёмка — pass.
- ✅ Волна 1 (блокеры прод): Redis обязателен, кардинальность метрик, закрытие self-register привилегий, SSE по ticket, честные лимиты реестров.
- ⏸️ Следующий шаг: прогнать волны тасков (код + ревью) отдельным прогоном; продуктовый код в прогоне спеки не менялся.

## Разработка по таскам + zh/hi — 09.09.2026

- ✅ Все 18 тасков выполнены, проверены двумя рецензентами (блокеров нет), закоммичены по одному на таск и запушены в `autopilot/m0-security-hardening`.
- ✅ Сюиты: backend 489 passed, frontend 180/180, build зелёный; ruff чисто; mypy-ошибки тасков закрыты (2 baseline не тронуты).
- ✅ Локали zh/hi: полные словари с паритетом EN, контент методологии переведён (доделка после скриншота), demo-стенд жив и не тронут.
- ✅ Память AGENTS.md переписана из кода, ADR 0028–0034 записаны, слепая приёмка по трём брифам — pass.
- ⚠️ Долг (неблокирующее, из крафт-ревью): дубли источников привилегий/размерности, тесты местами утверждают внутренности, ru/en +12 ключей без синхронного zh/hi (fallback EN); clamav-контейнер unhealthy — вне объёма, чинить отдельно.

## Доделка zh + отмена hi — 09.09.2026

- ✅ Хинди отменён пользователем (G02 dropped с цитатой); код T18 остаётся инертным без развития.
- ✅ Китайский доделан до конца (T19 `5b1e63e`): паритет zh/EN 2614/2614, ноль fallback вне allowlist, живой рендер методологии/уровней/лендинга/roadmap; данные пользователей — на языке ввода, как в EN (решение пользователя).
- ✅ Дельта-приёмка pass, прогон снова сдан и запушен; демо-стенд не тронут.

## Хинди убран из интерфейса — 09.09.2026

- ✅ T20 `b6e2366`: кнопка HI, реестр hi, мэппинги, словари и тесты hi удалены; запрос hi → дефолт ru без падений; RU/EN/ZH работают (приёмка подтверждает, демо живо).
- ✅ Прогон сдан повторно и запушен; итого 20 тасков, G02/G03 закрыты (отмена + удаление).

## Пароли и тоггл языка — 09.09.2026

- ✅ Диагноз: пароли не ломались — демо-учёток не было в dev-БД; плюс живой backend падал 500 на любом email (протухший venv без idna).
- ✅ T21 `37288c0`: досев 5 учёток без затирания; рестарт backend на свежем venv той же командой — живые входы 5/5 — 200, :3000/:8000 живы.
- ✅ T22 `f0a1e27`: тоггл RU/EN/ZH на /login и /register (шапка ЛК уже содержала); приёмка подтверждает, прогон сдан и запушен; итого 22 таска.
...
- ✅ T10 / CODE-01 reviewed and repaired: only 29 confirmed swallowed/fallback sites are counted; propagated middleware exceptions still produce HTTP 500 and are not counted as suppressed. Related tests 42 passed, Ruff PASS, Python 3.11 mypy 62 files PASS, full backend suite 748 passed / 1 existing warning. Evidence: `.autopilot/2026-09-23-production-stabilization--wip/evidence/T10-suppressed-exceptions.md`.
- ✅ T11 / CODE-02 implementation by OpenCode Space Bunny Free Max reviewed and committed (`3afbd94`): atomic increments and snapshot lock verified; AI tests 46 passed, Ruff and Python 3.11 mypy PASS, backend suite 748 passed / 1 existing warning. Executor stopped before its final contract, so results were independently rerun. Evidence: `.autopilot/2026-09-23-production-stabilization--wip/evidence/T11-ai-metrics-concurrency.md`.
- ✅ T12 / DB-03: complete index inventory and source-discovery regression test implemented by OpenCode Space Bunny Free Max, independently reviewed and verified. Focused static test 4 passed; Ruff and Python 3.11 mypy passed; full backend suite 752 passed / 1 existing warning. Craft review notes that key-expression/operator-class/`lists` details are not dynamically compared; tracked for final triage. Evidence: `.autopilot/2026-09-23-production-stabilization--wip/evidence/T12-db-index-inventory.md`.
