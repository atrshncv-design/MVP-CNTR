...
| R20 | «C4 — топология и зелёные гейты» (воспроизводимое окружение, зелёные backend/frontend/CI, решение SPOF / план HA) | in-ticket | Backend pytest/Ruff зелёные; T25 webpack build/238 tests PASS, Turbopack локально EPERM; CI блокируют mypy D03/D04 и npm audit D05-D07; topology T20 впереди | `evidence/T25-verification.md`, `evidence/T04-verification.md`, `evidence/CI-dependency-audit.md` |
| R21 | «Волна 0 — воспроизводимость и baseline» (изолированный worktree, SHA/status, версии, локальное окружение, `uv sync --locked --extra dev`, устранить CODE-03/CODE-06, честный baseline, без `\| tail`/`\|\| true`; гейты ruff/mypy/pytest/npm test/build; тесты только в test schema, никогда в production) | in-ticket | sync/Ruff/pytest/npm test PASS; mypy D03/D04 RED; T25 webpack build PASS, штатный Turbopack локально EPERM | `evidence/T02-verification.md`, `evidence/T25-verification.md`, `evidence/T04-verification.md` |
...
| D04 | Обнаружено в T04: mypy `[arg-type]` в `app/main.py:311` для validation exception handler | in-ticket | T27: согласовать контракт FastAPI/Starlette без смены ответа | `evidence/T04-verification.md` |
| D05 | CI frontend `npm audit` выявил `next` critical до запуска lint/tests/build | in-ticket | T28: минимальное обновление до patched Next; не скрывать advisory | `evidence/CI-dependency-audit.md` |
