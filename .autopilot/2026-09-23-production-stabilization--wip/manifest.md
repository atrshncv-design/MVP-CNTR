...
| R20 | «C4 — топология и зелёные гейты» (воспроизводимое окружение, зелёные backend/frontend/CI, решение SPOF / план HA) | in-ticket | Backend Python 3.11 gates GREEN; T28–T30 сняли frontend critical/high advisory, webpack build/238 tests PASS; CI lint D08 остаётся, Turbopack локально EPERM; topology T20 впереди | `evidence/T27-verification.md`, `evidence/T28-verification.md`, `evidence/T29-verification.md`, `evidence/T30-verification.md` |
| R21 | «Волна 0 — воспроизводимость и baseline» (изолированный worktree, SHA/status, версии, локальное окружение, `uv sync --locked --extra dev`, устранить CODE-03/CODE-06, честный baseline, без `\| tail`/`\|\| true`; гейты ruff/mypy/pytest/npm test/build; тесты только в test schema, никогда в production) | in-ticket | backend sync/Ruff/mypy/pytest PASS; frontend npm test/webpack build PASS, штатный Turbopack локально EPERM; audit high D05-D07 | `evidence/T27-verification.md`, `evidence/T25-verification.md` |
...
| D08 | После T28 полный frontend lint падает на `react-hooks/set-state-in-effect` в подсказке верификации организации | in-ticket | T31: убрать синхронный setState из effect без смены UX | `evidence/T28-verification.md` |
