...
| R20 | «C4 — топология и зелёные гейты» (воспроизводимое окружение, зелёные backend/frontend/CI, решение SPOF / план HA) | in-ticket | Backend pytest/Ruff зелёные; frontend tests 237 passed, build BLOCKED Google Fonts; mypy T04, topology T20 впереди | `evidence/T22-verification.md`, `evidence/T03-verification.md` |
| R21 | «Волна 0 — воспроизводимость и baseline» (изолированный worktree, SHA/status, версии, локальное окружение, `uv sync --locked --extra dev`, устранить CODE-03/CODE-06, честный baseline, без `\| tail`/`\|\| true`; гейты ruff/mypy/pytest/npm test/build; тесты только в test schema, никогда в production) | in-ticket | sync/Ruff/pytest/npm test PASS; mypy T04 BLOCKED, frontend build D02/T25 BLOCKED | `evidence/T02-verification.md`, `evidence/T03-verification.md` |
