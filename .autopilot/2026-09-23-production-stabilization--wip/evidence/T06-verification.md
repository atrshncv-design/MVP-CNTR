...
- Executor's full backend `uv run pytest -q` exceeded 300 seconds. Orchestrator reran the canonical command alone: 723 passed, 1 pre-existing warning in 463.25s (local test DB). Canonical mypy under repository Python 3.14 remains blocked by numpy stub syntax; compatible Python 3.11 mypy passes.
