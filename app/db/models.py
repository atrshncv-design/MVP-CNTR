...
class RefreshToken(Base):
    ...
    `family_id` (тикет 03): UUID-строка, генерируемая при выдаче. Все токены
    одной цепочки ротаций наследуют общий family_id — при reuse отозванного
    токена отзывается ВСЯ семья (два concurrent refresh не создают две
    действующие ветки).
    """
...
class Project(Base):
    ...
    # Назначенные верификатор (эксперт УГТ, роль regulating_organization) и
    # аудитор (роль auditor). Назначаются персоналом ЦНТР явно (тикет 02):
    # самоназначение запрещено, права на КТ даёт только назначение.
    assigned_verifier_id: Mapped[int | None] = mapped_column(
        BigInteger, ForeignKey("public.users.id"), index=True
    )
    assigned_auditor_id: Mapped[int | None] = mapped_column(
        BigInteger, ForeignKey("public.users.id"), index=True
    )
...
class UserProfile(Base):
    ...
    # Тикет 07 (security-remediation-audit): explicit consent на публикацию в
    # анонимном публичном каталоге специалистов. По умолчанию False —
    # специалист НЕ виден анонимам без активного согласия (152-ФЗ/ПДн);
    # авторизованные участники видят verified-профили как раньше.
    catalog_consent: Mapped[bool] = mapped_column(
        Boolean, nullable=False, default=False, server_default=false()
    )
...
# ─── Новостной раздел (тикет 05, спека §3.2) ────────────────────────────────
...
class NewsCategory(Base):
    """Категория новости (справочник; seed — миграция 0027)."""
