"""Новостной раздел (тикет 05, спека §3).

Публичные GET без токена (паттерн реестров — ``CurrentUserOptional``):
...
Статусы: draft → scheduled → published (§3.3); PATCH опубликованного
не меняет published_at (guard-период §3.5). Media — через
``app/services/file_storage.py`` (сигнатурный MIME, лимит 25 МБ).
``source != manual`` → 422, флаг ``created_automatically`` ставит только
будущий шлюз контент-завода (спека §5).
"""
...
from app.services.html_sanitize import sanitize_html, strip_tags
...
# post.cover_key — см. спека §3.2 «обложка»).
...
def _card_out(post: NewsPost) -> NewsCardOut:
    excerpt = strip_tags(post.content)[:EXCERPT_LIMIT]
    if len(strip_tags(post.content)) > EXCERPT_LIMIT:
        excerpt += "…"
    return NewsCardOut(
        id=post.id,
        title=post.title,
...
async def _ensure_tags(db: DBSession, names: list[str]) -> list[NewsTag]:
    """Мягкие теги: создаются при первом использовании (спека §3.2)."""
    result: list[NewsTag] = []
    for raw in names:
        name = raw.strip()[:160]
        if not name:
...
async def news_feed(
    ...
    """Публичная лента: только published, published_at DESC (спека §3.3)."""
...
async def my_news(db: DBSession, user: CurrentUser) -> list[NewsDetailOut]:
    """Свои новости (все статусы) для консоли автора (§3.3)."""
    rows = (
        (
            await db.execute(
                select(NewsPost)
...
async def create_news(
    ...
    """Создать новость (cntr_admin/cntr_manager; спека §5: source=manual)."""
...
async def update_news(
    ...
    """Редактирование (автор/cntr_admin); published_at не меняется (§3.5)."""
...
async def unpublish_news(
    ...
    """Снять с публикации (cntr_admin любую, автор — свою) → draft (§3.5)."""
    post = await _get_manageable_post(db, news_id, user)
    post.status = "draft"
...
async def delete_news(
    ...
    """Удалить (автор свою, cntr_admin любую); чистит media-файлы (§3.3)."""
    post = await _get_manageable_post(db, news_id, user)
