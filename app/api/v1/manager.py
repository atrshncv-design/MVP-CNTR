...
async def notify_managers(db: DBSession, type_: str, title: str, payload: dict[str, Any]) -> None:
...
async def decide_promotion(
    ...
    if payload.approve:
        ...
        # Тикет 02: подтверждение УГТ N → командные медали (ugt-N, отраслевая
        # sector-*, quality-проверки) всем активным участникам проекта на
        # момент события + мета-медали участников. Награждение идемпотентно:
        # повторное подтверждение того же уровня не дублирует записи; коммит
        # выполняется ниже — награды атомарны с решением менеджера.
