"""Каталог и витрина достижений (тикеты 01/03, спека §4.2, §4.6).

- ``GET /achievements/catalog`` — публичный каталог 66 медалей (тикет 01).
- ``GET /achievements/mine`` — персональная витрина авторизованного
  пользователя (тикет 03): выданные медали (что/когда/за какой
  проект/повторения) + прогресс до следующей ступени для пороговых
  медалей doc-*/m-*.
...
# «N/порог следующей ступени» (тикет 03, спека §4.6): документы и
# мета-медали. Ступени берутся из каталога (threshold не NULL).
...
async def achievements_mine(
    ...
    История — все записи user_achievements пользователя (свежие сверху):
    медаль, повторения (times), дата начисления, проект. Для пороговых
    ступеней групп documents/member дополнительно считается прогресс до
    следующей ступени каталога: current_count — последний подтверждённый
    порог группы (times выданной ступени), next_threshold — следующий
    порог из каталога (None, если достигнута максимальная ступень).
    """
    rows = (
        await db.execute(
...
async def project_achievements(
    ...
    Доступ: участники/менеджеры (паттерн require_project_access); анонимам
    медали видны только для публичного проекта (is_public=True) — по
    аналогии с публичным реестром проектов. Нарушителям — 404, чтобы не
    раскрывать существование закрытого проекта.
    """
