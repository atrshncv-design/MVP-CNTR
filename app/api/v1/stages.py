...
async def _enforce_quota(db: DBSession, project_id: int) -> None:
    """Мягкая квота файлов проекта (тикет 04, quota hook)."""
    from sqlalchemy import func as sqla_func

    current = await db.scalar(
        select(sqla_func.count(ProjectDocument.id)).where(
            ProjectDocument.project_id == project_id
        )
    )
    try:
        quota_hook(project_id, current or 0)
...
async def _trigger_application(
    ...
    result: dict[str, Any] = {"doc_id": doc.id, "request_id": None, "request_status": None}

    # Тикет 02: принятый документ → персональные медали (doc-first, ступени
    # 5/10/25/50/100, коллекционер). Награждение идемпотентно: повторная
    # версия того же документа не награждает повторно; счётчики ступеней
    # считают уникальные документы пользователя в проекте.
...
async def upload_stage_document_file(
    ...
    """
    await require_project_access(db, project_id, user)
    await _enforce_quota(db, project_id)
    project = await db.get(Project, project_id)
    if project is None:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "Проект не найден")
    if project.status != "published":
    ...
    # Тикет 04: bounded streaming read — oversize отклоняется до полной
    # буферизации (в памяти никогда не больше MAX_FILE_SIZE + чанк).
    try:
    ...
    # Тикет 04: orphan cleanup — если после записи объекта падает сканирование
    # или DB-операции ДО успешного commit, объект удаляется. scan_status="error"
    # — НЕ исключение: файл остаётся в карантине (evidence требует clean).
    committed = False
    try:
        ...
        result = await _trigger_application(db, project, doc, user)
        committed = True  # _trigger_application коммитит внутри
        return result
    except Exception:
        if not committed:
