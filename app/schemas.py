...
class ProjectOut(BaseModel):
    ...
    assigned_auditor_id: int | None = None
...
class ProjectAssignmentIn(BaseModel):
    """Назначение верификатора/аудитора на проект (тикет 02).

    `user_id=None` снимает назначение. Назначаемый пользователь должен иметь
    соответствующую роль (regulating_organization для verifier, auditor для
    auditor) — проверяется на стороне сервера.
    """

    assignment: Literal["verifier", "auditor"]
...
class RagDocumentIn(BaseModel):
    ...
    doc_type: str
    ugt_level: int | None = None
    # Тикет 06: raw_text ограничен сверху — embedding по unbounded тексту
    # дорогой; 100k символов заведомо выше реальных шаблонов/ГОСТ-фрагментов.
...
class RagSearchIn(BaseModel):
    # Тикет 06: bounds для query/top_k — unbounded embedding + KNN (finding #5).
...
class ChatIn(BaseModel):
    # Тикет 06: bounds — сообщение идёт в RAG-поиск (embedding) и LLM.
...
class PromotionRequestOut(BaseModel):
    ...
    verification_docs: list[dict[str, Any]] = []
...
class ProfileIn(BaseModel):
    ...
    skills: list[str] = []
    # Тикет 07: explicit consent на публикацию в анонимном каталоге.
    # None = «не менять» (PATCH не сбрасывает согласие, если поле не передано).
    catalog_consent: bool | None = None
