...
import { getVisibleMenuItems, MORE_MENU_ITEMS, MORE_MENU_LABEL } from "@/lib/more-menu";

/**
 * Кнопка «Больше функций» с выпадающей сеткой карточек.
 *
 * Поведение (тикет 01): открытие/закрытие по клику на кнопке, закрытие по
 ...
 * Сетка карточек (иконка + название + маленький бейдж «В разработке»)
 * наполняется из src/lib/more-menu.ts. Тикет 02: пункты фильтруются по ролям
 * текущей сессии через getVisibleMenuItems / allowedRolesFor (src/lib/roles.ts) —
 * ссылки, которые middleware запретит роли, в меню не показываются вовсе.
 */
export default function MoreFunctionsMenu({
  ...
  onNavigate?: () => void;
  /** Роли текущей сессии (slug-и из session.user.roles); undefined — без фильтрации. */
  userRoles?: string[];
}) {
  ...
  // Ролевая фильтрация (тикет 02): без сессии показываем полный список.
  const items =
    userRoles && userRoles.length > 0 ? getVisibleMenuItems(userRoles) : MORE_MENU_ITEMS;
