  ...
  return (
    <section className="tz-card p-6" data-od-id="stage-progress">
      ...
      {loading ? <div className="mt-5 h-20 animate-pulse rounded-xl bg-tz-soft" /> : error ? <div className="mt-5 rounded-xl bg-tz-danger-soft p-4 text-sm text-tz-danger">{error}</div> : <>
        ...
        <div className="mt-5 grid gap-3 md:grid-cols-2"><select className="tz-select" value={selectedId ?? ''} onChange={(e) => setSelectedId(Number(e.target.value))} aria-label="Требование для загрузки">{requirements.filter((r) => !r.uploaded).map((r) => <option key={r.id} value={r.id}>{r.title}</option>)}</select><input ref={fileRef} type="file" accept=".pdf,.docx,.xlsx,.png,.jpg,.jpeg" className="tz-input" aria-label="Файл подтверждения требования" onChange={(e) => { const f = e.target.files?.[0]; if (f) void upload(f); }} /></div>
