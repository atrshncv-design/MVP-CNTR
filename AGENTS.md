...
<!-- autopilot:start -->
# Платформа «Технозрелость» — памятка агенту (tier T1, верифицировано 2026-08-30)
...
- **AI-инфраструктура:** текущий LLM внешний (cloud OpenAI-compatible), эмбеддинги считаются CPU-hashing; локального neural inference нет, поэтому GPU в текущую закупку не нужен (`docs/СЕРВЕР-ТРЕБОВАНИЯ-2026.md:31-35`).
...
- **Утилиты:** `scripts/loadtest.py` (профиль 70/20/8/2 на 1000 VUs, флаги `--insecure`/`--bench-login`) + `scripts/security_check.py` (5 групп: secrets/deps/RBAC/IDOR/file-security); актуальный инфраструктурный отчёт `docs/СЕРВЕР-ТРЕБОВАНИЯ-2026.md`, также `docs/СЕРВЕР-ТРЕБОВАНИЯ.md` и `docs/ИМПОРТОЗАМЕЩЕНИЕ.md`; карта веток `docs/version-map.md`.
...
## Инфраструктура
- **Sizing/закупка (2026-08-30):** рекомендация `2×16` физических ядер, `128 ГБ ECC`, enterprise TLC+PLP и `10GbE` с оценкой стартового CAPEX `966 000 ₽`; это инженерная оценка, а не коммерческое предложение, и кандидат на 300–500 RPS только до production-like benchmark. При N-1 ожидается деградация примерно до 250–300 RPS (`docs/СЕРВЕР-ТРЕБОВАНИЯ-2026.md:15-29`).
- **Измеренная ёмкость:** актуальной успешной проверки 300–500 RPS и 10 000 соединений нет; старый результат около 55–61 RPS является устаревшим baseline, а текущий loadtest report — пустой stub (`docs/СЕРВЕР-ТРЕБОВАНИЯ-2026.md:92-96`). Цели 99,9%, RPO 5 минут и RTO 1 час также требуют benchmark/failover/restore proof.
...
- **Backend канонический:** `technozrelost-backend/` (`app/main.py`, `app/api/v1/*.py` ~25 роутеров, `app/services/*.py`, `app/db/models.py`, `alembic/versions/0001…0032`, `infra/`, `scripts/`, `tests/`). Архивная линия первого бэкенда (`app/`, `alembic/` в корне worktree) — только чтение.
