...
## Окно обслуживания (G18: один узел, без формального SLA)

Контур одноузловой, HA нет: на время перезапуска, миграции или
восстановления сервис недоступен. Формального SLA нет; действует окно:

- **Восстановление после аварии — в ближайший рабочий день**
  (пн–пт, 06:00–15:00 UTC / 09:00–18:00 MSK), цель — следующий рабочий день.
  Авария в выходной ждёт начала рабочего дня, если сервис не поднялся
  автоматическим рестартом (`restart: unless-stopped`).
- **Плановые работы** (выкладка, миграции, рестарт) — будни 06:00–09:00 UTC:
  после ночного бэкапа (03:15 UTC) и до начала рабочего дня; ручной бэкап
  перед работами — по P1 ниже.
- Дежурный узнаёт об аварии из Telegram-алертера (падение сервиса, диск,
  неудачный бэкап); без настроенных `TELEGRAM_BOT_TOKEN`/`TELEGRAM_CHAT_ID`
  окно не действует — проверять `docker compose ... logs alerter` вручную.
