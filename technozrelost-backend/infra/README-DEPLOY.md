...
| Frontend Next.js | 1.25 | 768M | SSR кабинетов; пик сборки ~2 ГБ покрывается запасом вне лимитов (см. ниже). Поднят с 0.75 (таск 08, приёмка 2026-09-17: page p95 3.06–3.5с при 50 concurrent против гейта 2с) за счёт простаивающих sidecar ниже |
...
| Prometheus | 0.4 | 384M | ретеншн ограничен: время `15d` и размер `10GB`; донор CPU для фронта (таск 08: в окне нагрузки только скрап раз в 15с) |
| Grafana | 0.10 | 256M | только по SSH-туннелю, вне публики; донор CPU для фронта (таск 08: в окне нагрузки простаивает) |
| nginx | 0.25 | 128M | точка входа TLS |
| Alerter | 0.05 | 128M | пробы readiness/диска/маркеров раз в 60с; донор CPU для фронта (таск 08) |
| Backup-timer | 0.10 | 128M | суточный снапшот вне пика; донор CPU для фронта (таск 08: в окне нагрузки спит до BACKUP_AT) |
| Wal-offsite | 0.10 | 128M | отправка WAL каждые 60 секунд; донор CPU для фронта (таск 08: копии килобайтные, вне горячего пути) |
| **Сумма** | **6.0** | **7680M = 7.5 ГиБ ≤ 8 ГиБ** | остаток 11 ГиБ — ОС, Docker и кратковременный пик сборки |

### Пул соединений БД (REPAIR 2026-09-17, норматив 50 concurrent)

`DB_POOL_SIZE=20` + `DB_MAX_OVERFLOW=35` = **55 соединений** на единственный
backend (`deploy.replicas: 1`, `DB_APP_REPLICAS=1`): 50 concurrent
аутентифицированных GET `/projects/registry` держат по **1 соединению**
(read-сессия общая для auth и данных через `ReadCurrentUser*`, раньше —
2 соединения и `QueuePool timeout` при пуле 30) + 5 headroom планировщику
новостей и readiness-пробам. Бюджет PostgreSQL (`max_connections=100`,
`postgresql-primary.conf`): `1×55 + резерв 10 = 65 < 100`. Память: 55
бэкендов PostgreSQL (~5–10 МБ каждый) укладываются в лимит Primary 1536M
(`shared_buffers 256M` + запас); со стороны backend 55 соединений asyncpg —
единицы МБ внутри лимита 1536M (образ ~682 МБ + запас). При росте нагрузки
сначала pgbouncer, а не эти цифры.
