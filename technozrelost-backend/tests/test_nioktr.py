...
def test_nioktr_sample_in_repo_meets_criteria() -> None:
    """Тикет 10: выборка в репозитории — ~200–500 карточек, размер < 50 МБ.

    Полный массив 414 МБ не читаем — проверяем только компактную выборку,
    которая уже лежит в data/nioktr_sample.json (prepare_nioktr).
    """
    sample = Path("data/nioktr_sample.json")
    if not sample.exists():
        return  # данные не примонтированы — тест пропускается

    data = json.loads(sample.read_text(encoding="utf-8"))
    cards = data.get("cards", [])
    assert 200 <= len(cards) <= 500, f"выборка: {len(cards)} карточек (ожидалось ~200–500)"
    assert sample.stat().st_size < 50 * 1024 * 1024, "выборка больше 50 МБ"
    # приоритет выборки — карточки is_ai_area / is_ai_usage (prepare_nioktr)
    ai = sum(1 for c in cards if c.get("is_ai_area") or c.get("is_ai_usage"))
    assert ai > 0, "в выборке нет AI-карточек"
    # у каждой карточки есть исполнитель с ОГРН (источник для organizations)
    with_ogrn = sum(1 for c in cards if (c.get("executor") or {}).get("ogrn"))
    assert with_ogrn > 0, "в выборке нет карточек с ОГРН исполнителя"
