...
def test_migration_0031_handles_garbage_date(client: TestClient) -> None:  # noqa: ARG001
    ...
    try:
        ...
        conn.execute(
            "DELETE FROM public.nioktr_cards "
            "WHERE registration_number LIKE 'TEST-GARBAGE-%'"
        )
        cases = [
            ("TEST-GARBAGE-EMPTY", ""),
        ...
        for reg, val in cases:
            conn.execute(
                "INSERT INTO public.nioktr_cards "
                "(registration_number, name, created_date) VALUES (%s,%s,%s)",
    ...
    try:
        rows = {
            ...
            for r in conn.execute(
                "SELECT registration_number, created_date "
                "FROM public.nioktr_cards "
                "WHERE registration_number LIKE 'TEST-GARBAGE-%'"
            ).fetchall()
        }
        # грязные → NULL
