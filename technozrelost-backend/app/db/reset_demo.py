...
from app.services.consent_service import REQUIRED_CONSENT_SLUGS
...
# хранят alembic_version/db_migration_log, consent_versions (юридические
# документы) сеются миграцией 0027 (тикет 04).
RESET_EXCLUDE = {
    "alembic_version",
    "db_migration_log",
    "roles",
...
async def seed_users() -> None:
    ...
    async with SessionLocal() as db:
        for spec in DEMO_USERS:
            ...
            # Обязательные согласия (тикет 04): демо-пользователи принимают
            # актуальные версии terms/privacy при сидировании — иначе
            # чувствительные операции (publish) блокируются pending-статусом.
            for slug in REQUIRED_CONSENT_SLUGS:
                ...
                if consent is not None:
                    db.add(
                        ConsentAcceptance(user_id=user.id, consent_version_id=consent.id)
                    )
        await db.commit()
