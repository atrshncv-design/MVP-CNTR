...
# Личные местоимения («я», «ты», «он»...) — тоже шум: без них запрос
# «Ты работаешь?» давал ILIKE-префильтр %ты% по всему корпусу.
RU_STOPWORDS: frozenset[str] = frozenset(
    {
        "и", "в", "на", "с", "по", "для", "как", "что", "это", "к", "о",
        "от", "из", "у", "же", "бы", "то", "так", "его", "ее", "её",
        "их", "мы", "вы", "они", "я", "ты", "он", "она", "оно",
        "но", "а", "или", "если", "когда",
...
SYNONYM_GROUPS: dict[str, tuple[str, ...]] = {
    ...
    # Уровни готовности: запрос «про УГТ 6» обязан находить разделы, где
    # аббревиатуры нет, а есть «уровень зрелости/готовности» (ГОСТ Р 58048).
    # «угт»/«trl» короткие — только точное совпадение; «готовност»/«зрелост» —
    # подстрокой по склонениям (готовности, зрелости). Фразы целиком механизм
    # одиночных токенов не покрывает — только их опорные слова.
    "syn_ugt": ("угт", "trl", "готовност", "зрелост"),
}
