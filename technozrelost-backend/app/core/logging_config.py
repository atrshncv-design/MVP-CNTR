...
# Покрытие (тикет 03): токены, пароли, ключи, а также одноразовые/MFA-коды
# (mfa_code/otp/totp/verification_code), сессионные и CSRF-токены.
_SECRET_KEY_RE = re.compile(
    r"(?i)\b(access[_-]?token|refresh[_-]?token|token|password|passwd|pwd|"
    r"secret|api[_-]?key|authorization|session[_-]?token|csrf[_-]?token|"
    r"id[_-]?token|mfa[_-]?code|otp|totp|one[_-]?time[_-]?code|"
    r"verification[_-]?code|2fa[_-]?code)\b([\"'\s=:]+)([^\s,}\"']+)"
)
...
def redact(text: str) -> str:
    ...
    Порядок важен: сначала Bearer-токены (иначе "Authorization: Bearer ***"
    маскирует только слово Bearer, оставляя сам токен), затем key=value по
    ключам password/token/email/secret/mfa/otp и т.п., затем e-mail адреса
    (включая email в URL/query-параметрах) — ЛЮБАЯ строка лога проходит
    через redact(); тела запросов не логируются вовсе (uvicorn их не пишет).
    """
