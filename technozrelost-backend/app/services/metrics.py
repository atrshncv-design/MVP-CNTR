...
def render(queue_pending: int = 0, storage_up: int = 0, storage_objects: int = 0) -> str:
    ...
    sec = security_metrics.snapshot()
    sec_help = {
        "auth_login_success_total": "Успешные входы (auth.login_success).",
        "auth_login_failed_total": "Неудачные входы (неверные учётные данные).",
        "auth_login_locked_total": "Попытки входа в деактивированный аккаунт.",
        "auth_register_total": "Саморегистрации (auth.register).",
        "files_uploads_total": "Загруженные файлы проектов (files.uploaded).",
        "files_downloads_total": "Скачивания файлов (files.downloaded).",
        "files_infected_total": "Файлы с вердиктом infected (ClamAV).",
        "files_rescans_total": "Повторные сканирования (files.rescanned).",
        "http_403_total": "HTTP-ответы 403 (Forbidden).",
        "http_429_total": "HTTP-ответы 429 (Too Many Requests).",
    }
    for name, help_text in sec_help.items():
        lines.append(f"# HELP technozrelost_{name} {help_text}")
        lines.append(f"# TYPE technozrelost_{name} counter")
        lines.append(f"technozrelost_{name} {sec[name]}")
    ...
    async def __call__(self, scope: dict[str, Any], receive: Any, send: Any) -> None:
        ...
        async def wrapped_send(message: dict[str, Any]) -> None:
            if message.get("type") == "http.response.start":
                ...
                security_metrics.http_denied(status["code"])
            await send(message)
